...
- 基础池按**8个月**底部窗口摊算（weekly: 34批，biweekly: 17批）；触发门槛 score ≤ **35**（约等于 200dMA 跌破时的周期分水位，先于信号池启动）
- ETF纪元备选路径：MVRV ratio 因机构买盘结构性抬底，准极端/极端底部添加 FGI 驱动备选路径（见 signals.js detectSignalLevel）
- 基础池触发门槛 **35**，信号池触发门槛 **28**，两池先后有序：200dMA 跌破时基础定投先启动，多指标共振后信号池再加仓
...
| 3 | 画像展示（12类 + 默认宣言模板 + Codex API 可选生成） | ✅ 已完成 |
...
- Codex API 直连使用 `anthropic-dangerous-direct-browser-access: true` header，调用 Codex-haiku-4-5-20251001，Token 由用户承担。
...
- **Codex API**：可选，所有LLM调用消耗用户自己的Token，不经过产品服务器
